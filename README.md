...
Alternatively, subscribe to Judge0 CE/Extra CE through RapidAPI and add `JUDGE0_API_URL`, `JUDGE0_RAPIDAPI_KEY`, and `JUDGE0_RAPIDAPI_HOST` in Vercel's environment variables. The RapidAPI key is used only by the backend; never add it to `VITE_` variables, frontend code, or Git.
